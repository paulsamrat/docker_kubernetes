...
(cd /usr/local/isp/bin && ./infasetup.sh defineWorkerNode -dn Domain -nn $worker_node_name -na 10.75.149.36:31100 -dg $master_domain_ip:$master_domain_port -un $master_domain_ac_username -pd $master_domain_ac_password -rf nodeoptions.xml)

(cd /usr/local/isp/bin && ./infacmd.sh updategatewayinfo -dn Domain -dg $master_domain_ip:$master_domain_port)
